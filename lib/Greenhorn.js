...
(function() {
  var AniCycle, AniSprite, AudioContext, EventEmitter, Greenhorn, KEYS, Line, Sound, Sprite, TextSprite, Timer, env, _audioContext, _masterID, _masterUpdate, _mixin, _ref,
  ...
  this.Point = (function() {
    function Point(x, y, org_x, org_y) {
      this.x = x;
      this.y = y;
      this.org_x = org_x;
      this.org_y = org_y;
    ...
    Point.prototype.get = function(what) {
      if (what === 'x') {
        ...
        return this.org_x.toFixed(2);
      } else if (what === 'org_y') {
        return this.org_y.toFixed(2);
      } else if (what === 'a') {
        return Math.atan2(this.y - this.org_y, this.x - this.org_x);
      } else if (what === 'dist') {
        return Math.sqrt(Math.pow(this.y - this.org_y, 2) + Math.pow(this.x - this.org_x, 2));
      } else {
    ...
    Point.prototype.set = function(what, to) {
      var diff, _x, _y;
      if (what === 'x') {
        this.x = to + this.org_x;
      } else if (what === 'y') {
        this.y = to + this.org_y;
      } else if (what === 'org_x') {
        diff = to - this.org_x;
        this.org_x = to;
        this.x += diff;
        ...
        this.x = _x + this.org_x;
        this.y = _y + this.org_y;
      } else if (what === 'dist') {
    ...
    Point.prototype.change = function(what, step) {
      ...
      if (what === 'x') {
        ...
        _x = this.get('dist') * Math.cos(step + this.get('a'));
        _y = this.get('dist') * Math.sin(step + this.get('a'));
        this.x = _x + this.org_x;
        this.y = _y + this.org_y;
      } else if (what === 'dist') {
        _x = (this.get('dist') + step) * Math.cos(this.get('a'));
        _y = (this.get('dist') + step) * Math.sin(this.get('a'));
        this.x = _x + this.org_x;
        this.y = _y + this.org_y;
      } else {
  ...
  Line = (function() {
    function Line(p1, p2) {
      ...
      } else if (this.p1.get('x') === this.p2.get('x') && this.p1.get('y') > this.p2.get('y')) {
        _ref1 = [this.p2, this.p1], this.p1 = _ref1[0], this.p2 = _ref1[1];
      }
    }

    Line.prototype.get = function(what, roundOff) {
      if (roundOff == null) {
        roundOff = true;
      }
      if (what === 'slope' || 'm') {
        if (this.p1.get('x') === this.p2.get('x')) {
          ...
          if (roundOff) {
            return ((this.p2.y - this.p1.y) / (this.p2.x - this.p1.x)).toFixed(2);
          } else {
            return (this.p2.y - this.p1.y) / (this.p2.x - this.p1.x);
          }
        }
      } else if (what === 'y-intercept' || 'b') {
        if (this.p1.get('x') === this.p2.get('x')) {
          return void 0;
        } else {
          if (roundOff) {
            return -((this.p2.y - this.p1.y) / (this.p2.x - this.p1.x) * this.p1.x + this.p1.y).toFixed(2);
          } else {
            return -(this.p2.y - this.p1.y) / (this.p2.x - this.p1.x) * this.p1.x + this.p1.y;
          }
        }
      } else {
    ...
    Line.prototype._contains = function(pt) {
      ...
      if ((this.p1.get('x') === (_ref = this.p2.get('x')) && _ref === pt.get('x'))) {
        if ((this.p1.get('y') <= (_ref1 = pt.get('y')) && _ref1 <= this.p2.get('y'))) {
          return true;
        }
      } else if ((this.p1.get('x') <= (_ref2 = pt.get('x')) && _ref2 <= this.p2.get('x'))) {
        if ((this.get('m', false) * pt.x + this.get('b', false)).toFixed(2) === pt.get('y')) {
          return true;
        }
      }
      return false;
    ...
    Line.prototype._intersection = function(other) {
      ...
      if (this.get('m') === other.get('m')) {
        ...
        _x = other.p1.x;
        _y = this.get('m', false) * _x + this.get('b', false);
      } else {
        _x = (other.get('b', false) - this.get('b', false)) / (this.get('m', false) - other.get('m', false));
        _y = this.get('m', false) * _x + this.get('b', false);
      }
  ...
  Sprite = (function(_super) {
    ...
    function Sprite(config) {
      var angles, key, magnitudes, pt, value, _i, _len, _ref;
      if (config == null) {
        config = {};
      }
      ...
      for (key in config) {
        ...
        if (key.match(/(^distance$|^speed$|^rate$)/i)) {
          ...
          if (typeof value === 'function') {
            ...
            this.on(key.slice(2), value[0], value[1]);
          }
          ...
          for (_i = 0, _len = value.length; _i < _len; _i++) {
            pt = value[_i];
            this._bnd.push(new Point(pt.x, pt.y, config.x, config.y));
          }
        }
      }
      if (this._bnd == null) {
        this._bnd = [];
        this._bnd.push(new Point(-config.width / 2, config.height / 2, config.x, config.y));
        this._bnd.push(new Point(config.width / 2, config.height / 2, config.x, config.y));
        this._bnd.push(new Point(config.width / 2, -config.height / 2, config.x, config.y));
        this._bnd.push(new Point(-config.width / 2, -config.height / 2, config.x, config.y));
      }
    ...
    Sprite.prototype.set = function(what, to, _emit) {
      var diff, k, newCollision, oldCollision, proxy, pt, side, v, _bottom, _i, _j, _k, _l, _left, _len, _len1, _len2, _len3, _len4, _len5, _len6, _len7, _m, _n, _o, _p, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _right, _top;
      if (_emit == null) {
        _emit = true;
      }
      if (what.match(/^x$/)) {
        ...
        this.set('y', to - Math.abs(_top.get('y') - _top.get('org_y')), false);
      } else if (what.match(/^bottom$/)) {
        _bottom = this._bnd[0];
        _ref4 = this._bnd;
        ...
        this.set('y', to + Math.abs(_bottom.get('y') - _bottom.get('org_y')), false);
      } else if (what.match(/^right$/)) {
        _right = this._bnd[0];
        _ref5 = this._bnd;
        ...
        this.set('x', to - Math.abs(_right.get('x') - _right.get('org_x')), false);
      } else if (what.match(/^left$/)) {
        _left = this._bnd[0];
        _ref6 = this._bnd;
        ...
        this.set('x', to + Math.abs(_left.get('x') - _left.get('org_x')), false);
        ...
        for (_p = 0, _len7 = to.length; _p < _len7; _p++) {
          pt = to[_p];
          this._bnd.push(new Point(pt.x, pt.y, this._pos.x, this._pos.y));
        }
      } else {
    ...
    Sprite.prototype.change = function(what, step, _emit) {
      var k, proxy, pt, v, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2;
      if (_emit == null) {
        _emit = true;
      }
      if (what.match(/^x$/)) {
        this._pos.x += step / env.FRAME_RATE;
        _ref = this._bnd;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          pt = _ref[_i];
        ...
        this._pos.y += step / env.FRAME_RATE;
        _ref1 = this._bnd;
        for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
          pt = _ref1[_j];
        ...
        this._pos.a += step / env.FRAME_RATE;
        _ref2 = this._bnd;
        for (_k = 0, _len2 = _ref2.length; _k < _len2; _k++) {
          pt = _ref2[_k];
        ...
        proxy = {
          dx: this.get('distance', false) * Math.cos(step),
          dy: this.get('distance', false) * Math.sin(step)
        };
        this.change('_pos', proxy, false);
      } else if (what.match(/^motAngle$/)) {
        proxy = {
          ddx: this.get('speed', false) * Math.cos(step),
          ddy: this.get('speed', false) * Math.sin(step)
        };
        this.change('_mot', proxy, false);
      } else if (what.match(/^accAngle$/)) {
        proxy = {
          dddx: this.get('rate', false) * Math.cos(step),
          dddy: this.get('rate', false) * Math.sin(step)
        };
        this.change('_acc', proxy, false);
    ...
    Sprite.prototype.collidesWith = function(other) {
      var i, mousePos, myLine, myLines, otherLine, otherLines, pt, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _m, _n, _o, _ref, _ref1, _ref2;
      if (other === 'mouse') {
        if (this._dis.visible) {
          if (this.distanceTo('mouse') <= this.get('radius', false)) {
            ...
            for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
              ...
              if (i === this._bnd.length - 1) {
                myLines.push(new Line(pt, this._bnd[0]));
              } else {
                myLines.push(new Line(pt, this._bnd[i + 1]));
              }
            }
            for (_j = 0, _len1 = myLines.length; _j < _len1; _j++) {
              myLine = myLines[_j];
              for (_k = 0, _len2 = otherLines.length; _k < _len2; _k++) {
                otherLine = otherLines[_k];
                if (!(myLine.p1 === otherLine.p1 || myLine.p1 === otherLine.p2)) {
                  if (!(myLine.p2 === otherLine.p1 || myLine.p2 === otherLine.p2)) {
    ...
    Sprite.prototype.highlight = function() {
      return this.on('draw:after', function() {
        var i, pt, _i, _len, _ref;
        this._dis.context.save();
        this._dis.context.lineWidth = 3;
        this._dis.context.strokeStyle = 'white';
        this._dis.context.beginPath();
        this._dis.context.moveTo(this._bnd[0].x, this._bnd[0].y);
        _ref = this._bnd;
        for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
          pt = _ref[i];
          if (i !== 0) {
            this._dis.context.lineTo(pt.x, pt.y);
          }
        }
