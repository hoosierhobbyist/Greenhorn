...
(function() {
  ...
  Sprite = (function() {
    ...
    Sprite.prototype._update = function() {
      var bounds, event, key, listeners, tokens, _ref;
      this.emit('update');
      this.change('_mot', this._acc);
      this.change('_pos', this._mot);
      bounds = {
        top: this._dis.context.canvas.height / 2,
        bottom: -this._dis.context.canvas.height / 2,
        right: this._dis.context.canvas.width / 2,
        left: -this._dis.context.canvas.width / 2
      };
      if (this.get('bottom') > bounds.top) {
        this.emit('offTop');
      }
      if (this.get('top') < bounds.bottom) {
        this.emit('offBottom');
      }
      if (this.get('left') > bounds.right) {
        this.emit('offRight');
      }
      if (this.get('right') < bounds.left) {
        this.emit('offLeft');
      }
      if (this.get('top') >= bounds.top) {
        this.emit('hitTop');
      }
      if (this.get('bottom') <= bounds.bottom) {
        this.emit('hitBottom');
      }
      if (this.get('right') >= bounds.right) {
        this.emit('hitRight');
      }
      if (this.get('left') <= bounds.left) {
        this.emit('hitLeft');
      }
      _ref = this._events;
      for (event in _ref) {
